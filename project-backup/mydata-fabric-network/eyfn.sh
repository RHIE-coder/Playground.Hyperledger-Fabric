...
function printHelp () {
  ...
  echo
  echo "	eyfn.sh generate -c mydata-channel"
  echo "	eyfn.sh up -c mydata-channel -s couchdb"
  echo "	eyfn.sh up -l node"
  echo "	eyfn.sh down -c mydata-channel"
  echo
...
# be parked in the mydata_platform folder titled ``crypto-config``.
...
# If mydata_platform wasn't run abort
if [ ! -d crypto-config ]; then
  echo
  echo "ERROR: Please, run mydata_platform.sh first."
  echo
  exit 1
fi
