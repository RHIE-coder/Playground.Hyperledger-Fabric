...
# channel previously setup in the mydata_platform tutorial and install the
...
set -x
peer channel fetch 0 $CHANNEL_NAME.block -o orderer.mydata.com:7050 -c $CHANNEL_NAME --tls --cafile $ORDERER_CA >&log.txt
res=$?
set +x
cat log.txt
