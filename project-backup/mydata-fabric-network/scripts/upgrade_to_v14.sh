...
echo
echo "Upgrade your first network (mydata_platform) from v1.3.x to v1.4.x end-to-end test"
echo
CHANNEL_NAME="$1"
DELAY="$2"
LANGUAGE="$3"
...
addCapabilityToChannel() {
        ...
        if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
                peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer.mydata.com:7050 --cafile $ORDERER_CA
                res=$?
                set +x
        else
                set -x
                peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer.mydata.com:7050 --tls true --cafile $ORDERER_CA
                res=$?
                set +x
        fi
